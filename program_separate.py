...
def main():
    ...
    for compound in all_compounds:
        ...
        rep_samples, t_resampled  = getInterpolatedDataSeparate(compound_df)
        ...
        for rep_sample,rep_name in zip(rep_samples,target_rep):
            ...
            first_period, two_peak = findFirstPeriod2(rep_good, t_resampled, window_size=100, start_time=12)
        ...
        period_dict[compound_name]['average period'] = round(float(first_period), 2)
        print(f'Done {compound_name}')

    result_period = pd.DataFrame.from_dict(period_dict, orient='index').reset_index()
    result_period.columns = ['Compound Name', 'Rep1 Period', 'Rep2 Period', 'Rep3 Period', 'Average Period']
